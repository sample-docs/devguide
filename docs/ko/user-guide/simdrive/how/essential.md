# SIM: Dirve 필수 정보
시뮬레이터 사용의 이해도를 높이기 위해 기준 좌표계 및 모델 규격과 같이 기본적으로 숙지해야 할 정보에 대하여 설명합니다.

---

## 좌표계
시뮬레이터의 차량, 센서, 맵에서 기준으로 사용하는 좌표계(Coordinate System)를 설명합니다.

### 위치 좌표계
시뮬레이터에서 차량의 위치 및 차량에 장착하는 센서의 위치를 나타내는 좌표계에 대해 설명합니다. 아래 좌표계를 기준으로 나타냅니다.

#### 차량 위치 좌표계
시뮬레이터에서 차량 위치는 맵의 원점을 기준으로 ENU 좌표계로 나타냅니다. 또한 차량의 위치 좌표깂은 멥의 원점에서부터 해당 차량의 후륜축 중심점까지의 거리로 계산합니다.
...
<figcaption><center><b>그림 3. 시뮬레이터의 센서 인지 환경</b></center></figcaption>
<br>
<table>
    <tr>
...
<figcaption><b>그림 4. 시뮬레이터 맵 좌표계</b></figcaption>

???+ tip
    시뮬레이터 내 ENU 좌표 값을 확인하려면 원하는 화면 위치에 마우스 스크롤을 클릭합니다. 자세한 사용법은 [유용한 사용팁의 좌표 표시 기능](../useful/#_2)을 참고하십시오.

<br>
ENU 좌표값을 실제 지구 상에 위치하는 좌표값으로 변환하고자 할 때는 세계에서 통용되고 있는 투영좌표계인 [UTM 좌표계](https://ko.wikipedia.org/wiki/UTM_%EC%A2%8C%ED%91%9C%EA%B3%84)를 기준으로 계산합니다.
...
## 차량 모델 사양
 Ego 차량 및 Sur 차량 모델에 적용된 실제 차량 사양에 대해 설명합니다.

### Ego 차량

???+ tip
    Ego 차량의 모델 별 제원에 대한 상세한 정보는 이전 공식 가이드 [Ego Vehicle Setting List](https://help-morai-sim.scrollhelp.site/morai-sim-standard-kr/vehicle-specification#VehicleSpecification-EgoVehicleSettingList)를 참고하십시오.

### Sur 차량

???+ tip
    Sur 차량의 모델 별 제원에 대한 상세한 정보는 이전 공식 가이드 [Sur Vehicle Setting List](https://help-morai-sim.scrollhelp.site/morai-sim-standard-kr/vehicle-specification#VehicleSpecification-SurroundVehicleSettingList)를 참고하십시오.