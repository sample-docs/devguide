...
(() => {
  ...
  self.fetch || (self.fetch = function(e, n) {
    return n = n || {}, new Promise(function(t, r2) {
      var s = new XMLHttpRequest(), o = [], u = {}, a = function e2() {
        ...
        }, clone: e2, headers: { keys: function() {
          return o;
        }, entries: function() {
          return o.map(function(e3) {
            return [e3, s.getResponseHeader(e3)];
          });
        }, get: function(e3) {
          return s.getResponseHeader(e3);
        }, has: function(e3) {
          return null != s.getResponseHeader(e3);
        } } };
      };
      for (var i in s.open(n.method || "get", e, true), s.onload = function() {
        s.getAllResponseHeaders().toLowerCase().replace(/^(.+?):/gm, function(e2, n2) {
          u[n2] || o.push(u[n2] = n2);
        }), t(a());
      }, s.onerror = r2, s.withCredentials = "include" == n.credentials, n.headers)
  ...
  ***************************************************************************** */
  ...
  var Observable = function() {
    function Observable52(subscribe) {
      if (subscribe) {
        this._subscribe = subscribe;
      }
    }
    Observable52.prototype.lift = function(operator) {
      var observable2 = new Observable52();
      observable2.source = this;
      observable2.operator = operator;
      return observable2;
    };
    Observable52.prototype.subscribe = function(observerOrNext, error, complete) {
      var _this = this;
    ...
    Observable52.prototype._trySubscribe = function(sink) {
      try {
    ...
    Observable52.prototype.forEach = function(next, promiseCtor) {
      var _this = this;
    ...
    Observable52.prototype._subscribe = function(subscriber) {
      var _a2;
      return (_a2 = this.source) === null || _a2 === void 0 ? void 0 : _a2.subscribe(subscriber);
    };
    Observable52.prototype[observable] = function() {
      return this;
    };
    Observable52.prototype.pipe = function() {
      var operations = [];
      for (var _i = 0; _i < arguments.length; _i++) {
    ...
    Observable52.prototype.toPromise = function(promiseCtor) {
      var _this = this;
      promiseCtor = getPromiseCtor(promiseCtor);
      return new promiseCtor(function(resolve, reject) {
        var value;
    ...
    Observable52.create = function(subscribe) {
      return new Observable52(subscribe);
    };
    return Observable52;
  }();
  ...
  (() => {
    ...
    function d(...s) {
      return s.reduce((n, e) => n.then(() => new Promise((r2) => {
        let t = document.createElement("script");
        t.src = e, t.onload = r2, document.body.appendChild(t);
      })), Promise.resolve());
    }
    var o = class extends EventTarget {
      constructor(e) {
        ...
        this.e = (e2, r3, t, i, m) => {
          if (r3 === `${this.url}`) {
            let a = new ErrorEvent("error", { message: e2, filename: r3, lineno: t, colno: i, error: m });
            this.dispatchEvent(a), this.onerror && this.onerror(a);
          }
        };
        let r2 = document.createElement("iframe");
        r2.hidden = true, document.body.appendChild(this.iframe = r2), this.w.document.open(), this.w.document.write(`<html><body><script>postMessage=${c};importScripts=${d};addEventListener("error",ev=>{parent.dispatchEvent(new ErrorEvent("error",{filename:"${e}",error:ev.error}))})<\/script><script src=${e}?${+Date.now()}><\/script></body></html>`), this.w.document.close(), onmessage = this.m, onerror = this.e, this.r = new Promise((t, i) => {
  ...
  function renderSearchDocument(document2, flag) {
    ...
    return /* @__PURE__ */ h("a", { href: `${url}`, class: "md-search-result__link", tabIndex: -1 }, /* @__PURE__ */ h(
      ...
      parent2 > 0 && /* @__PURE__ */ h("div", { class: "md-search-result__icon md-icon" }),
      parent2 > 0 && /* @__PURE__ */ h("h1", null, document2.title),
      parent2 <= 0 && /* @__PURE__ */ h("h2", null, document2.title),
  ...
  function renderSearchResultItem(result) {
    ...
    const docs = [...result];
    const config4 = configuration();
    const parent2 = docs.findIndex((doc) => {
    ...
    const factory$ = defer(() => {
      ...
      if (import_clipboard.default.isSupported()) {
        if (el.closest(".copy") || feature("content.code.copy") && !el.closest(".no-copy")) {
          ...
          parent2.id = `__code_${sequence++}`;
          parent2.insertBefore(
            renderClipboardButton(parent2.id),
            el
          );
        }
      }
  ...
  function fetchScripts() {
    return typeof mermaid === "undefined" || mermaid instanceof Element ? watchScript("https://unpkg.com/mermaid@9.4.3/dist/mermaid.min.js") : of(void 0);
  }
    ...
    return defer(() => {
      ...
      push$.pipe(
        map(() => {
          ...
          return backgroundColor.match(/\d+/g).map((value) => (+value).toString(16).padStart(2, "0")).join("");
        })
  ...
  function setupInstantLoading({ location$: location$2, viewport$: viewport$2 }) {
    const config4 = configuration();
    ...
    instant$.subscribe(location$2);
    const response$ = location$2.pipe(
      startWith(getLocation()),
      distinctUntilKeyChanged("pathname"),
      skip(1),
      switchMap(
        (url) => request(url).pipe(
          catchError(() => {
            setLocation(url);
            return EMPTY;
          })
    ...
    const dom = new DOMParser();
    const document$2 = response$.pipe(
      switchMap((res) => res.text()),
      switchMap((res) => {
        const document2 = dom.parseFromString(res, "text/html");
    ...
    popstate$.pipe(map(getLocation)).subscribe(location$2);
    location$2.pipe(
      startWith(getLocation()),
      bufferCount(2, 1),
      switchMap(
        ([prev, next]) => prev.pathname === next.pathname && prev.hash !== next.hash ? of(next) : EMPTY
      )
    ).subscribe((url) => {
      var _a2, _b;
    ...
    document$2.pipe(withLatestFrom(location$2)).subscribe(([, url]) => {
      var _a2, _b;
      if (history.state !== null || !url.hash) {
        window.scrollTo(0, (_b = (_a2 = history.state) == null ? void 0 : _a2.y) != null ? _b : 0);
      } else {
  ...
  function setupVersionSelector({ document$: document$2 }) {
    ...
    document$2.pipe(switchMap(() => current$)).subscribe((current) => {
      ...
      if (outdated === null) {
        outdated = true;
        const ignored = [((_a2 = config4.version) == null ? void 0 : _a2.default) || ["latest"]].flat();
        main:
      ...
      switchMap(
        (body) => defer(() => {
          ...
          return of([...table].reduce((index2, [anchor, target]) => {
            ...
            let parent2 = target.offsetParent;
            for (; parent2; parent2 = parent2.offsetParent)
  ...
  if (feature("navigation.instant"))
    setupInstantLoading({ location$, viewport$ }).subscribe(document$);
  var _a;
